		...
		echo wp_kses_post(
			SFWD_LMS::get_template(
				...
				array(
					...
					'message'      => sprintf( esc_html_x( '%s complete. Results are being recorded. This may take several minutes. This page will refresh automatically. Please do not refresh or navigate away from the page.', 'placeholder: Quiz', 'learndash' ), LearnDash_Custom_Label::get_label( 'quiz' ) ),
				)
			)
		);
		?>
...
<div style="display: none;" class="wpProQuiz_results">
	<h4 class="wpProQuiz_header"><?php esc_html_e( 'Results', 'learndash' ); ?></h4>
	<div class="el-qr-wrap">
		<div class="el-qr-top">
			<?php if ( $quiz->isToplistActivated() ) { ?>
				<div class="d-inline-flex user-current-rank">
					<span class="qr-label"><?php echo esc_html__( 'Your Position', 'elumine' ); ?></span>
					<p>
						<?php
						echo getLeaderboardPosition($quiz->getID());
						?>
					</p>
				</div>
			<?php }
				...
				echo wp_kses_post(
					SFWD_LMS::get_template(
						...
						array(
							...
							'message'      => '<span class="qr-label">'. esc_html_x( 'answered correctly', 'learndash' ) .'</span><p>' . sprintf( esc_html_x( '%1$s of %2$s ', 'placeholder: correct answer, question count', 'learndash' ), '<span class="wpProQuiz_correct_answer">0</span>', '<span>' . $question_count . '</span>' ) . '</p>',
			...
			if ( ! $quiz->isHideResultQuizTime() ) {
				?>
				<p class="wpProQuiz_quiz_time d-inline-flex">
				<?php
					echo wp_kses_post(
						SFWD_LMS::get_template(
							'learndash_quiz_messages',
							array(
								...
								'message'      => '<label class="qr-label">' . esc_html_x( 'Your time', 'learndash' ) .'</label>'. sprintf( esc_html_x( '%s', 'placeholder: quiz time.', 'learndash' ), '<span></span>' ),
							)
						)
					);
				?>
			...
			if ( ! $quiz->isHideResultPoints() ) {
				?>
				<p class="wpProQuiz_points d-inline-flex">
				<?php
					echo wp_kses_post(
						SFWD_LMS::get_template(
							'learndash_quiz_messages',
							array(
								...
								'message'      => '<label class="qr-label">' . esc_html_x( 'You have reached', 'learndash' ) .'</label>'.sprintf( esc_html_x( ' %1$s of %2$s point(s), (%3$s)', 'placeholder: points earned, points total', 'learndash' ), '<span>0</span>', '<span>0</span>', '<span>0</span>' ),
					...
					echo wp_kses_post(
						SFWD_LMS::get_template(
							...
							array(
								...
								'message'      => '<label class="qr-label">' . esc_html_x( 'Earned Point(s):', 'learndash' ) .'</label>'.sprintf( esc_html_x( ' %1$s of %2$s, (%3$s)', 'placeholder: points earned, points total, points percentage', 'learndash' ), '<span>0</span>', '<span>0</span>', '<span>0</span>' ),
								'placeholders' => array( '0', '0', '0' ),
							)
						)
					);
			...
			echo wp_kses_post(
				SFWD_LMS::get_template(
					...
					array(
						...
						'message'      => sprintf( esc_html_x( 'Essay(s) Pending: %1$s (Possible Point(s): %2$s)', 'placeholder: number of essays, possible points', 'learndash' ), '<span>0</span>', '<span>0</span>' ),
